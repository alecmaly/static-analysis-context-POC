...
parser.add_argument('--search-functions-regex', '-sf', type=str, help='The regex to search for in function names')
...
parser.add_argument('--skip-not-in-parents', '-snip', action='store_true', help='Skip functions not in parents')
...
## NOTES
# To fix:
    # - combine search-functions-regex + match-file-regex
    # - fix match-file-regex to output matches

args = parser.parse_args()

def print_hyperlink(text, url):
    # Construct the ANSI escape code for a hyperlink
    hyperlink = f"\033]8;;{url}\033\\{text}\033]8;;\033\\"
    return hyperlink
...
if args.search_functions_regex:
    for f in functions:
        if not re.search(args.search_functions_regex, f.get('qualifiedName', f.get('functionName', '')) + f['decorator'], re.IGNORECASE):
            continue
        
        print(f"{f['filepath']}: {f.get('qualifiedName', f.get('functionName', ''))} {f['decorator']}")
...
for filepath in functions_map.keys():
    if args.match_file_regex and not re.search(args.match_file_regex, filepath, re.IGNORECASE):
        continue

    if args.ignore_file_regex and re.search(args.ignore_file_regex, filepath, re.IGNORECASE):
        continue
    ...
    if args.search_regex and not re.search(args.search_regex, content, re.IGNORECASE):
    ...
    for i, line in enumerate(content_arr):
        if args.search_regex and re.search(args.search_regex, line, re.IGNORECASE):
            # print(f"{filepath}:{i+1}: {line.strip()}")
            in_lines.append(i+1)
    ...
    for f_id in in_functions:
        ...
        f_str = f['qualifiedName'] if f.get('qualifiedName') else f.get('functionName', '')
        ...
        for i, line in enumerate(in_functions[f_id]):
            ...
            url = f"vscode://file{filepath.split('#')[0]}:{line}"
            
            print(f"\t{print_hyperlink(line, url)}: {line_content}")
            

    if not args.skip_not_in_parents:
        print(colorama.Fore.RED + "Not in functions:" + colorama.Style.RESET_ALL)
        for f in not_in_functions:
            f_str = f_str = f['qualifiedName'] if f.get('qualifiedName') else f.get('functionName', '')
            print(f"{f['filepath']}: {colorama.Fore.YELLOW}{f_str}{colorama.Style.RESET_ALL} {f['decorator']}")
    print("\n\n")
